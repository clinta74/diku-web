...
public sealed class CoordinateIsolationTests
{
    ...
    public void Domain_references_no_other_project_in_this_solution()
    {
        ...
        var referenced = DomainAssembly
            ...
            .Select(a => a.Name!)
            .Where(name => name.StartsWith("DikuWeb", StringComparison.Ordinal))
            .ToList();

        Assert.True(
            referenced.Count == 0,
            "DikuWeb.Domain must reference no other DikuWeb project. Found: "
    ...
    public void Command_handlers_cannot_reach_the_layout_service()
    {
        ...
        var commandTypes = EngineAssembly.GetTypes()
            .Where(t => t.Namespace?.StartsWith("DikuWeb.Engine.Commands", StringComparison.Ordinal) == true)
    ...
    public void The_layout_service_is_the_only_type_producing_map_coordinates()
    {
        ...
        var producers = EngineAssembly.GetTypes()
            .Where(t => !IsCompilerGenerated(t))
            .Where(t => t.Namespace?.StartsWith("DikuWeb.Engine.Presentation", StringComparison.Ordinal) == true)
            .Select(t => t.Name)
            .ToList();
