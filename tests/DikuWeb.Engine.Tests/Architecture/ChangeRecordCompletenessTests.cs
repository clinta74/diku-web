...
namespace DikuWeb.Engine.Tests.Architecture;
...
public sealed class ChangeRecordCompletenessTests
{
    ...
    {
        var source = Source("src", "DikuWeb.Engine", "Mutations", "WorldChange.cs");
    ...
    public void The_applier_reads_every_field_of_every_change()
    {
        AssertEveryFieldIsRead(
            ...
            "src", "DikuWeb.Engine", "Mutations", "WorldMutationApplier.cs");
    }

    /// <summary>Miss one here and the field works until the server restarts.</summary>
    /// <remarks>
    /// The writer lives in <c>DikuWeb.Server</c> while the change records live in
    /// <c>DikuWeb.Engine</c>, so this reaches across a project boundary. That is the point: the
    ...
    public void The_writer_reads_every_field_of_every_change()
    {
        AssertEveryFieldIsRead(
            "writer",
            "WorldWriter",
            "src", "DikuWeb.Server", "Building", "WorldWriter.cs");
    }
}
